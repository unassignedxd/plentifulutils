...
public final class VoidUtil {
    ...
    public static void spawnParticlesInMiddleChunk(World world, ChunkPos pos)
    {
        ...
        ((WorldServer) world).spawnParticle(EnumParticleTypes.ENCHANTMENT_TABLE, true, x, y, z, 1000, 0, 4, 0, 1D);
    }

    public static void spawnParticlesRandomWithinChunk(World world, ChunkPos pos, int amt)
    {
        for(int i = 0; i < amt; i++)
        {
            double x = ((pos.x) << 4) + (0 + (8 - 0) * world.rand.nextDouble()); //todo remove minus, its additive (0->16) not (-8->8) also: create item that warns player of unstable void events. Also create model for void hole, possibly make the size get smaller/bigger depending on the void in the area, and decay once void is stabliized. over all clean up code.
    ...
    public static void setBlockStateInMiddleChunk(World world, ChunkPos pos, Block block)
    {
        double x = ((pos.x) << 4) + 8.5;
        double z = ((pos.z) << 4) + 8.5;
        int y = world.getHeight((int)x,(int)z) + 5;

        ((WorldServer) world).setBlockState(new BlockPos(x, y, z), block.getDefaultState());
    }
}
